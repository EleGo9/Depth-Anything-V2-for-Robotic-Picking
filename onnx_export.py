...
import torch.onnx
import numpy as np
from relative_depth.depth_anything_v2.dpt import DepthAnythingV2
import onnxruntime as ort
import onnx


class DepthModelWrapper(nn.Module):
    """Wraps DepthAnythingV2 to always output 4D (B, 1, H, W).
    Optionally normalizes to [0, 1] dividing by depth_scale."""
...
def main():
    ...
    parser.add_argument('--depth-scale', type=float, default=None,
                        help='If set, wraps the model to normalize output to [0,1] by dividing by this value')
    ...
    depth_anything = DepthModelWrapper(depth_anything, depth_scale=args.depth_scale)
    if args.depth_scale is not None:
        print(f"Wrapping model with normalization: output / {args.depth_scale}")
